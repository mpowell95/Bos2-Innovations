...
/* Caret that shows or hides a section's slide list without changing the
   ...
   The gutter is reserved on every row, not just the ones that have a caret.
   Reserving it only where a caret exists left checkboxes at two different
   indents — a section keeps its gutter even while unchecked, when the caret is
   hidden — so the left edge came out ragged and looked broken. One column for
   the caret, one for the checkbox, on every row. */
.builder-item{ position:relative; padding-left:36px; }
.nested-caret{
...
color:#fff; background:var(--fiord); border-radius:5px;
